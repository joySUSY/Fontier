...
export const getGlyphs = (fontData) => {
  const font = parse(fontData);
  const glyphs = [];
  for (let i = 0; i < font.numGlyphs; i++) {
    const glyph = font.glyphs.get(i);
    if (glyph.unicode !== undefined) {
      glyphs.push({ name: glyph.name, unicode: glyph.unicode });
    }
  }
  return glyphs;
};

export const getGlyphByName = (fontData, name) => {
  const font = parse(fontData);
  const glyphIndex = font.charToGlyphIndex(name);
  return font.glyphs.get(glyphIndex);
};

export const getGlyphComponents = (fontData, name) => {
  const font = parse(fontData);
  const glyph = getGlyphByName(fontData, name);
  const components = [];
  const path = glyph.getPath(0, 0, 500);
  path.commands.forEach((command) => {
    ...
    const { x, y } = command;
    if (command.type === 'M') {
      components.push({ type: 'M', data: [x, y] });
    } else if (command.type === 'L') {
      components.push({ type: 'L', data: [x, y] });
    } else if (command.type === 'Q') {
      const { x1, y1 } = command;
      components.push({ type: 'Q', data: [x1, y1, x, y] });
    } else if (command.type === 'C') {
      const { x1, y1, x2, y2 } = command;
      components.push({ type: 'C', data: [x1, y1, x2, y2, x, y] });
    }
  });
  return components;
};
